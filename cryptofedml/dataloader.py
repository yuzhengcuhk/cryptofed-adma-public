...
class EpisodeDataset(data.Dataset):
    ...
    def __init__(self, root, phase='train', n_shot=5, n_eval=15, transform=None):
        ...
        # 应该在这里查看有多少个label，然后根据这个label划分不同worker数据集的大小
...
def prepare_data(args):
    ...
    train_set = EpisodeDataset(args.data_root, 'train', args.n_shot, args.n_eval,
                               transform=transforms.Compose([
                                   ...
                                   transforms.ToTensor(),
                                   normalize]))
    # train1_set = EpisodeDataset(args.data_root, 'train1', args.n_shot, args.n_eval,
    #                            transform=transforms.Compose([
    #                                transforms.RandomResizedCrop(args.image_size),
    #                                transforms.RandomHorizontalFlip(),
    #                                transforms.ColorJitter(
    ...
    # val1_set = EpisodeDataset(args.data_root, 'val1', args.n_shot, args.n_eval,
    #                          transform=transforms.Compose([
    #                              transforms.Resize(args.image_size * 8 // 7),
    #                              transforms.CenterCrop(args.image_size),
    #                              transforms.ToTensor(),
    ...
    #
    # train1_loader = data.DataLoader(train1_set, num_workers=args.n_workers, pin_memory=args.pin_mem,
    #                                batch_sampler=EpisodicSampler(len(train1_set), args.n_class, args.episode))
    ...
    # val1_loader = data.DataLoader(val1_set, num_workers=2, pin_memory=False,
    #                              batch_sampler=EpisodicSampler(len(val1_set), args.n_class, args.episode_val))
    ...
    #return train_loader, train1_loader, val_loader, val1_loader, test_loader
